...
pcall <- pca_cs %>% 
  rename(pcall = pc,
         pcall_2 = pc2,
         pcall_3 = pc3)
var_label(pcall$pcall) <- NULL
var_label(pcall$pcall_2) <- NULL
var_label(pcall$pcall_3) <- NULL
...
# Check
table(!is.na(ses_cs$pcall2016_1))
table(!is.na(ses_cs$pcall2018_1))
...
# SAVE ---------------
saveRDS(ses_cs,paste0(path_harmonization_folder,"/guatemala/working/ses_cs for COHORTS.RDS"))
write_dta(ses_cs,paste0(path_harmonization_folder,"/guatemala/working/ses_cs for COHORTS.dta"),version=12)
