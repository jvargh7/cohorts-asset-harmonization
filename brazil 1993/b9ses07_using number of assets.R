...
pca_vars = c("c_crowding","n_car","d_computer","d_duplexfridge","d_dvd","n_housekeeper","d_refrigerator","n_television",
             "d_vaccumcleaner","d_washingmachine","p_pipedwater","n_bathroom")
...
pca_df_unimputed_nassets <- pca_df
...
saveRDS(polychoric_out,paste0(path_harmonization_folder,"/brazil 1993/working/polychoric_out nassets.RDS"))





pca_df_unimputed <- readRDS(paste0(path_harmonization_folder,"/brazil 1993/working/pca_df_unimputed.RDS"))
pca_df_unimputed_nassets[,c("pc", paste0("pc",c(2,3)))] <- polychoric_out$scores[,1:3]

r = paste0(
  sprintf(abs(cor(pca_df_unimputed$pc,pca_df_unimputed_nassets$pc,method = "spearman")),fmt = "%0.2f")
)
...
pca_df_unimputed_nassets <- label_variables(pca_df_unimputed_nassets,cohorts_labels = b9pcall_labels,overwrite_var_label = FALSE)
...
# SAVE ---------------
saveRDS(pca_df_unimputed_nassets,paste0(path_harmonization_folder,"/brazil 1993/working/pca_df_unimputed_nassets.RDS"))
write_dta(pca_df_unimputed_nassets,paste0(path_harmonization_folder,"/brazil 1993/working/pca_df_unimputed_nassets.dta"),version=12)
