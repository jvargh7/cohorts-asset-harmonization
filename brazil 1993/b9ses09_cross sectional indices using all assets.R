...
for (row in 1:nrow(ses_spearman_cs)){
  ...
  if(y == 2011){
    exclude_vars = c(exclude_vars,"p_pipedwater")
  }
  
  if(y == 2015){
    exclude_vars = c(exclude_vars,"d_vaccumcleaner")
  }
  ...
  temp_pca_df <- pcall %>% 
    ...
    dplyr::select(nquest,year, one_of(temp_pca_vars)) %>% 
    arrange(nquest) 

  harmonized_index <- readRDS(paste0(path_harmonization_folder,"/brazil 1993/working/pca_df_unimputed.RDS")) %>% 
    dplyr::filter(year == y) %>% 
    dplyr::select(nquest,year, pc,pc2,pc3)  %>% 
    arrange(nquest)
...
saveRDS(pca_cs,paste0(path_harmonization_folder,"/brazil 1993/working/pca_cs.RDS"))
write_dta(pca_cs,paste0(path_harmonization_folder,"/brazil 1993/working/pca_cs.dta"),version=12)

ses_spearman_cs %>%
  pivot_wider(names_from="Var1",values_from=r) %>%
  write.csv(.,paste0(path_harmonization_folder,"/brazil 1993/working/correlation with cs all items indices.csv"),row.names = FALSE)

# Each row is a PC1 loading
ses_pca_obj %>%
  distinct(year,iteration,item,.keep_all = TRUE) %>% 
  pivot_wider(names_from="item",values_from="PC1") %>%
  write.csv(.,paste0(path_harmonization_folder,"/brazil 1993/working/cs all items indices loadings.csv"),row.names = FALSE)

# Each value is variable importance from dropping none (=all) or one of the variables
ses_pca_imp %>%
  write.csv(.,paste0(path_harmonization_folder,"/brazil 1993/working/cs all items indices proportion of variance.csv"),row.names = FALSE)
