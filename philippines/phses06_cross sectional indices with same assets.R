...
ses_spearman_cs %>%
  ...
  write.csv(.,paste0(path_harmonization_folder,"/philippines/working/correlation with cs indices.csv"),row.names = FALSE)

# Each row is a PC1 loading
ses_pca_obj %>%
  distinct(year,iteration,item,.keep_all = TRUE) %>% 
  pivot_wider(names_from="item",values_from="PC1") %>%
  write.csv(.,paste0(path_harmonization_folder,"/philippines/working/cs indices loadings.csv"),row.names = FALSE)

# Each value is variable importance from dropping none (=all) or one of the variables
ses_pca_imp %>%
  write.csv(.,paste0(path_harmonization_folder,"/philippines/working/cs indices proportion of variance.csv"),row.names = FALSE)
