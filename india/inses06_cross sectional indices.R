...
pca_df <- readRDS(paste0(path_harmonization_folder,"/india/working/pca_df_unimputed.RDS")) %>% 
  ...
  dplyr::select(year,id,one_of(pca_vars),pc,pc2,pc3)
...
for (row in 1:nrow(ses_spearman_cs)){
  ...
  temp_x = temp_pca_df %>% 
    dplyr::select(-year,-id,-pc,-pc2,-pc3) %>% 
...
ses_spearman_cs %>%
  ...
  write.csv(.,paste0(path_harmonization_folder,"/india/working/correlation with cs indices.csv"),row.names = FALSE)
...
ses_pca_obj %>%
  ...
  write.csv(.,paste0(path_harmonization_folder,"/india/working/cs indices loadings.csv"),row.names = FALSE)

# Each value is variable importance from dropping none (=all) or one of the variables
ses_pca_imp %>%
  write.csv(.,paste0(path_harmonization_folder,"/india/working/cs indices proportion of variance.csv"),row.names = FALSE)
