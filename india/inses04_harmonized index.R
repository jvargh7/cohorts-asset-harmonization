...
pca_df <- pcall %>% 
  dplyr::filter(year %in% years) %>% 
  dplyr::select(year,id,one_of(pca_vars)) %>% 
...
source(paste0(path_harmonization_repo,"/india/in_pca_df_imputation.R"))
...
# zero_var_indices <- caret::nearZeroVar(pca_df)
# names_zero_var <- names(pca_df)[zero_var_indices]
# 
# pca_vars = pca_vars[!pca_vars %in% names_zero_var]


x = pca_df %>% 
  dplyr::select(-year,-id,-non_na_assets) %>% 
...
saveRDS(polychoric_out,paste0(path_harmonization_folder,"/india/working/polychoric_out.RDS"))
...
pca_df_unimputed <- label_variables(pca_df_unimputed,cohorts_labels = inpcall_labels,overwrite_var_label = FALSE)
...
# SAVE ---------------
saveRDS(pca_df_unimputed,paste0(path_harmonization_folder,"/india/working/pca_df_unimputed.RDS"))
write_dta(pca_df_unimputed,paste0(path_harmonization_folder,"/india/working/pca_df_unimputed.dta"),version=12)
