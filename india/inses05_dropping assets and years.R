...
for (row in 1:nrow(ses_pearson)){
  ...
  temp_x = temp_pca_df %>% 
    dplyr::select(-year,-id,-pc,-pc2,-pc3) %>% as.matrix()
...
ses_pearson %>%
  ...
  write.csv(.,paste0(path_harmonization_folder,"/india/working/drop asset x year.csv"),row.names = FALSE)
...
ses_pearson2 %>%
  pivot_wider(names_from="Var1",values_from=r) %>%
  write.csv(.,paste0(path_harmonization_folder,"/india/working/drop asset x asset.csv"),row.names = FALSE)
...
ses_pearson3 %>%
  pivot_wider(names_from="Var1",values_from=r) %>%
  write.csv(.,paste0(path_harmonization_folder,"/india/working/drop year x year.csv"),row.names = FALSE)