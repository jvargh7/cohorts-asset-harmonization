...
pca_df <- pcall %>% 
  dplyr::filter(year %in% years) %>% 
  dplyr::select(year,bttid,one_of(pca_vars)) %>% 
...
x = pca_df %>% 
  dplyr::select(-year,-bttid,-non_na_assets) %>% as.matrix()

polychoric_out <- polychoric_pca(x,pca_vars)   
saveRDS(polychoric_out,paste0(path_harmonization_folder,"/south africa/working/polychoric_out.RDS"))
...
pca_df_unimputed <- label_variables(pca_df_unimputed,cohorts_labels = sapcall_labels,overwrite_var_label = FALSE)
...
# SAVE ---------------
saveRDS(pca_df_unimputed,paste0(path_harmonization_folder,"/south africa/working/pca_df_unimputed.RDS"))
write_dta(pca_df_unimputed,paste0(path_harmonization_folder,"/south africa/working/pca_df_unimputed.dta"),version=12)
